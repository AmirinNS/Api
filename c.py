...
def getTrendData(stockName):
    ...
    if result != None:
        ...
        # Load the dataset
        profitLength = len(result['Profit MA'])
        profit_Data_X = np.arange(profitLength).reshape((profitLength, 1))
        profit_Data_Y = result['Profit MA']
        # Create linear regression model
        profitLinearModel = linear_model.LinearRegression()
        profitLinearModel.fit(profit_Data_X, profit_Data_Y)
        profit_Data_Y_predict = profitLinearModel.predict(profit_Data_X)
        # Convert NumPy to List to solve JSON not serializable error
        profit_Data_Y_predict = profit_Data_Y_predict.tolist()
        # Add profit_Data_Y_predict to result object
        result['profit_regression'] = profit_Data_Y_predict
        
        # Repeat for Revenue

        revenueLength = len(result['Revenue MA'])
        revenue_Data_X = np.arange(revenueLength).reshape((revenueLength, 1))
        revenue_Data_Y = result['Revenue MA']
        revenueLinearModel = linear_model.LinearRegression()
        revenueLinearModel.fit(revenue_Data_X, revenue_Data_Y)
        revenue_Data_Y_predict = revenueLinearModel.predict(revenue_Data_X)
        revenue_Data_Y_predict = revenue_Data_Y_predict.tolist()
        result['revenue_regression'] = revenue_Data_Y_predict

        return template('chart', value=json.dumps(result))
    else:
        return None

@route('/api/stock/trend/test/<stockName>')
def getTrendDataTest(stockName):
    result = read_csv(stockName)
    if result != None:
        arrayData_Y = []
        # Load the dataset
        length = len(result['Profit MA'])
        data_X = np.arange(length).reshape((length, 1))
        data_Y = result['Profit MA']
        # data_Y = [float(i) for i in data_Y]
        linearModel = linear_model.LinearRegression()
        linearModel.fit(data_X, data_Y)
        data_Y_predict = linearModel.predict(data_X)
        arrayData_Y = data_Y_predict
